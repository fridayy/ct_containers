...
container_info() ->
  #{<<"AppArmorProfile">> => <<>>,
    ...
    <<"Config">> =>
    #{<<"AttachStderr">> => false, <<"AttachStdin">> => false,
      <<"AttachStdout">> => false,
      <<"Cmd">> =>
      [<<"/usr/sbin/mosquitto">>, <<"-c">>,
        <<"/mosquitto/config/mosquitto.conf">>],
      ...
      <<"OnBuild">> => null, <<"OpenStdin">> => false,
      <<"StdinOnce">> => false, <<"Tty">> => false, <<"User">> => <<>>,
      <<"Volumes">> =>
      #{<<"/mosquitto/data">> => #{}, <<"/mosquitto/log">> => #{}},
      <<"WorkingDir">> => <<>>},
    <<"Created">> => <<"2021-10-17T17:58:08.492019391Z">>,
    <<"Driver">> => <<"overlay2">>, <<"ExecIDs">> => null,
    <<"GraphDriver">> =>
    #{<<"Data">> =>
    #{<<"LowerDir">> =>
    ...
    #{<<"PidMode">> => <<>>,
      ...
      [<<"/proc/bus">>, <<"/proc/fs">>, <<"/proc/irq">>, <<"/proc/sys">>,
        <<"/proc/sysrq-trigger">>],
      <<"BlkioWeightDevice">> => null, <<"CpuRealtimeRuntime">> => 0,
      <<"GroupAdd">> => null, <<"VolumeDriver">> => <<>>,
      <<"CpuShares">> => 0, <<"PidsLimit">> => null,
      <<"SecurityOpt">> => null, <<"CapDrop">> => null,
      <<"ConsoleSize">> => [0, 0],
      <<"CpuQuota">> => 0, <<"AutoRemove">> => false,
      <<"CgroupParent">> => <<>>, <<"DeviceCgroupRules">> => null,
      <<"BlkioDeviceWriteBps">> => null, <<"CpusetMems">> => <<>>,
      <<"MemoryReservation">> => 0, <<"ShmSize">> => 67108864,
      <<"BlkioDeviceReadBps">> => null, <<"DnsOptions">> => null,
      <<"Isolation">> => <<>>,
      <<"MaskedPaths">> =>
      ...
      <<"IOMaximumBandwidth">> => 0, <<"VolumesFrom">> => null,
      <<"NanoCpus">> => 0,
      <<"RestartPolicy">> =>
      #{<<"MaximumRetryCount">> => 0, <<"Name">> => <<>>},
      <<"UTSMode">> => <<>>, <<"BlkioWeight">> => 0,
      <<"OomKillDisable">> => null, <<"CgroupnsMode">> => <<"private">>,
      <<"CpusetCpus">> => <<>>, <<"Privileged">> => false,
      <<"Devices">> => null, <<"BlkioDeviceReadIOps">> => null,
      <<"Cgroup">> => <<>>, <<"IOMaximumIOps">> => 0,
      <<"PortBindings">> => null, <<"CpuPeriod">> => 0,
      <<"DeviceRequests">> => null, <<"Dns">> => null,
      <<"MemorySwappiness">> => null, <<"CpuPercent">> => 0,
      <<"ExtraHosts">> => null, <<"Binds">> => null, <<"UsernsMode">> => <<>>,
      <<"NetworkMode">> => <<"default">>, <<"Runtime">> => <<"runc">>,
      <<"ReadonlyRootfs">> => false, <<"CapAdd">> => null,
      <<"KernelMemory">> => 0, <<"Ulimits">> => null, <<"MemorySwap">> => 0,
      <<"ContainerIDFile">> => <<>>, <<"KernelMemoryTCP">> => 0,
      <<"CpuCount">> => 0, <<"OomScoreAdj">> => 0,
      <<"PublishAllPorts">> => false, <<"Links">> => null,
      <<"LogConfig">> =>
      #{<<"Config">> => #{}, <<"Type">> => <<"json-file">>},
      <<"DnsSearch">> => null, <<"IpcMode">> => <<"private">>,
      <<"CpuRealtimePeriod">> => 0, <<"BlkioDeviceWriteIOps">> => null,
      <<"Memory">> => 0},
    <<"HostnamePath">> =>
    ...
    <<"Id">> =>
    <<"7853cf0bbe920586f8e1c73fa9c9da9ffc2751efbca2561a7b9143b6f24e5641">>,
    <<"Image">> =>
    ...
    [#{<<"Destination">> => <<"/mosquitto/data">>,
      <<"Driver">> => <<"local">>, <<"Mode">> => <<>>,
      <<"Name">> =>
      <<"660c342f7785f123d54cebe21a7a83328786d44796ca9c999e11c4193d48a34b">>,
      <<"Propagation">> => <<>>, <<"RW">> => true,
      <<"Source">> =>
      ...
      #{<<"Destination">> => <<"/mosquitto/log">>,
        <<"Driver">> => <<"local">>, <<"Mode">> => <<>>,
        <<"Name">> =>
        <<"fac05ec81465c39135c162a64e9ff68e877229ac743be7bb73a429927bff7405">>,
        <<"Propagation">> => <<>>, <<"RW">> => true,
        <<"Source">> =>
    ...
    #{<<"Bridge">> => <<>>,
      ...
      <<"GlobalIPv6PrefixLen">> => 0, <<"HairpinMode">> => false,
      <<"IPAddress">> => <<"172.17.0.2">>, <<"IPPrefixLen">> => 16,
      <<"IPv6Gateway">> => <<>>, <<"LinkLocalIPv6Address">> => <<>>,
