...
%% @end
start(FixedDelay) ->
  gen_server:call(?SERVER, {start, FixedDelay}, (FixedDelay + ?CLIENT_CONNECT_TIMEOUT_MS + 5000)).
...
handle_info({tcp_closed, _Port}, #state{listener_socket = LSock, reply_to = From} = State) ->
  gen_server:reply(From, {ok, client_disconnected}),
  gen_tcp:close(LSock),
  {noreply, State}.
